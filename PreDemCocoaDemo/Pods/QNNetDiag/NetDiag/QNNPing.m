...
- (NSString *)description {
  if (_code == 0 || _code == kQNNRequestStoped) {
    return [NSString
        stringWithFormat:@"%d packets transmitted, %ld packets received, %f "
                         @"packet loss time %fms\n round-trip "
                         @"min/avg/max/stddev = %.3f/%.3f/%.3f/%.3f ms",
                         (int)(_count + _loss), (long)_count,
                         (double)_loss * 100 / (_count + _loss), _totalTime,
                         _minRtt, _avgRtt, _maxRtt, _stddev];
  }
...
enum { kQNNICMPTypeEchoReply = 0, kQNNICMPTypeEchoRequest = 8 };
...
static ICMPPacket *build_packet(uint16_t seq, uint16_t identifier) {
  ...
  snprintf((char *)packet->payload, kQNNPacketSize - sizeof(ICMPPacket),
           "qiniu ping test %d", (int)seq);
...
static BOOL isValidResponse(char *buffer, int len, int seq, int identifier) {
  ...
  uint16_t calculatedChecksum =
      in_cksum(icmpPtr, len - ((char *)icmpPtr - buffer));

  return receivedChecksum == calculatedChecksum &&
         icmpPtr->type == kQNNICMPTypeEchoReply && icmpPtr->code == 0 &&
...
- (int)sendPacket:(ICMPPacket *)packet
  ...
  ssize_t sent = sendto(sock, packet, (size_t)_size, 0, (struct sockaddr *)addr,
...
- (QNNPingResult *)buildResult:(NSInteger)code
  ...
  if (code != 0 && code != kQNNRequestStoped) {
    return [[QNNPingResult alloc] init:code
                                    ip:ip
                                  size:_size
                                   max:0
                                   min:0
...
- (void)run {
    ...
    if (host == NULL || host->h_addr == NULL) {
      ...
      if (_complete != nil) {
        [QNNQue async_run_main:^(void) {
          QNNPingResult *result = [[QNNPingResult alloc] init:-1006
                                                           ip:nil
                                                         size:_size
                                                          max:0
                                                          min:0
  ...
  NSTimeInterval *durations =
      (NSTimeInterval *)calloc(sizeof(NSTimeInterval) * _count, 1);
  int index = 0;
  int r = 0;
  ...
  do {
    ...
    if (r == 0) {
      ...
      [self.output
          write:[NSString
                    stringWithFormat:
                        @"%d bytes from %s: icmp_seq=%ld ttl=%d time=%f ms\n",
                        size, inet_ntoa(addr.sin_addr), (long)index, ttl,
                        duration * 1000]];
      durations[index - loss] = duration * 1000;
    } else {
      [self.output
          write:[NSString
                    stringWithFormat:@"Request timeout for icmp_seq %ld\n",
                                     (long)index]];
      loss++;
    }
...
+ (instancetype)start:(NSString *)host
  ...
  QNNPing *ping = [[QNNPing alloc] init:host
                                   size:size
                                 output:output
                               complete:complete
                               interval:interval
