...
- (NSInteger)sendAndRecv:(int)sendSock
  ...
  QNNTraceRouteRecord *record =
      [[QNNTraceRouteRecord alloc] init:ttl count:TraceMaxAttempts];
  for (int try = 0; try < TraceMaxAttempts; try ++) {
    NSDate *startTime = [NSDate date];
    ssize_t sent = sendto(sendSock, cmsg, sizeof(cmsg), 0,
                          (struct sockaddr *)addr, sizeof(struct sockaddr));
    if (sent != sizeof(cmsg)) {
      err = errno;
      NSLog(@"error %s", strerror(err));
      [self.output
          write:[NSString stringWithFormat:@"send error %s\n", strerror(err)]];
      break;
    }
    ...
    if (FD_ISSET(icmpSock, &readfds) > 0) {
      ssize_t res = recvfrom(icmpSock, buff, sizeof(buff), 0,
                             (struct sockaddr *)&storageAddr, &n);
      if (res < 0) {
        err = errno;
...
- (void)run {
  ...
  [self.output
      write:[NSString stringWithFormat:@"traceroute to %@ ...\n", _host]];
    ...
    if (host == NULL || host->h_addr == NULL) {
      ...
      if (_complete != nil) {
        [QNNQue async_run_main:^(void) {
          QNNTraceRouteResult *result =
              [[QNNTraceRouteResult alloc] init:-1006 ip:nil content:nil];
          _complete(result);
        }];
      }
      return;
  ...
  if (-1 == fcntl(recv_sock, F_SETFL, O_NONBLOCK)) {
    ...
    if (_complete != nil) {
      [QNNQue async_run_main:^(void) {
        QNNTraceRouteResult *result = [[QNNTraceRouteResult alloc]
               init:-1
  ...
  do {
    ...
    [self sendAndRecv:send_sock recv:recv_sock addr:&addr ttl:ttl ip:&ip];
           ...
           [[NSDate date] timeIntervalSinceDate:startDate] <= 20);
...
+ (instancetype)start:(NSString *)host
             ...
             complete:(QNNTraceRouteCompleteHandler)complete
               maxTtl:(NSInteger)maxTtl {
  QNNTraceRoute *t = [[QNNTraceRoute alloc] init:host
                                          output:output
                                        complete:complete
                                          maxTtl:maxTtl];
