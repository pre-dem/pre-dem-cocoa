...
- (NSString *)description {
  if (_code == 0 || _code == kQNNRequestStoped) {
    return [NSString
        stringWithFormat:@"tcp connect min/avg/max = %.3f/%.3f/%.3fms",
...
- (void)run {
    ...
    if (host == NULL || host->h_addr == NULL) {
      ...
      if (_complete != nil) {
        dispatch_async(dispatch_get_main_queue(), ^(void) {
          _complete([self buildResult:-1006
                                   ip:nil
                            durations:nil
                                 loss:0
                                count:0
  ...
  do {
    ...
    if (r == 0) {
      [self.output
          write:[NSString stringWithFormat:@"connected to %s:%lu, %f ms\n",
                                           inet_ntoa(addr.sin_addr),
                                           (unsigned long)_port,
                                           duration * 1000]];
    } else {
      [self.output
          write:[NSString stringWithFormat:
                              @"connect failed to %s:%lu, %f ms, error %d\n",
      ...
      _complete([self
          buildResult:code
            ...
            durations:intervals
                 loss:loss
                count:index
            totalTime:[[NSDate date] timeIntervalSinceDate:startDate] * 1000]);
      free(intervals);
    });
  }
}
...
- (QNNTcpPingResult *)buildResult:(NSInteger)code
  ...
  if (code != 0 && code != kQNNRequestStoped) {
    return [[QNNTcpPingResult alloc] init:code
                                       ip:ip
                                      max:0
                                      min:0
                                      avg:0
...
+ (instancetype)start:(NSString *)host
             ...
             complete:(QNNTcpPingCompleteHandler)complete {
  return
      [QNNTcpPing start:host port:80 count:3 output:output complete:complete];
}
