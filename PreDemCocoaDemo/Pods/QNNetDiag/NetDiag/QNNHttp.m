...
- (NSString *)description {
  ...
  if (_body != nil) {
    NSString *str =
        [[NSString alloc] initWithData:_body encoding:NSUTF8StringEncoding];
    if (str == nil) {
      bodySummary = @"not utf8 string body";
    }
  }
  return [NSString stringWithFormat:@"code:%ld duration:%f body:%@",
                                    (long)_code, _duration, bodySummary];
}
...
- (void)run {
  ...
  if (_output) {
    ...
    [_output
        write:[NSString stringWithFormat:@"complete duration:%f status %ld\n",
  ...
  if (httpError != nil) {
    QNNHttpResult *result = [[QNNHttpResult alloc] init:httpError.code
                                                     ip:[self reserveUrlToIp]
                                               duration:duration
                                                headers:nil
                                                   body:nil];
    _complete(result);
  ...
  QNNHttpResult *result = [[QNNHttpResult alloc] init:response.statusCode
                                                   ip:[self reserveUrlToIp]
                                             duration:duration
