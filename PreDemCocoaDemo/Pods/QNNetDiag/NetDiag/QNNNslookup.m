...
static NSArray *query_ip(res_state res, const char *host) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (t == ns_t_a) {
      ...
      int x = ns_name_uncompress(answer, &(answer[len]), ns_rr_rdata(rr),
...
- (void)run {
  ...
  if (_dnsServer == nil) {
    ...
    r = setup_dns_server(
        &res, [_dnsServer cStringUsingEncoding:NSASCIIStringEncoding]);
  }
  if (r != 0) {
    return;
  }
  ...
  NSArray *records =
      query_ip(&res, [_domain cStringUsingEncoding:NSUTF8StringEncoding]);
...
+ (instancetype)start:(NSString *)domain
               ...
               output:(id<QNNOutputDelegate>)output
             complete:(QNNNslookupCompleteHandler)complete {
  QNNNslookup *instance = [[QNNNslookup alloc] init:domain
                                             server:dnsServer
                                             output:output
                                           complete:complete];
