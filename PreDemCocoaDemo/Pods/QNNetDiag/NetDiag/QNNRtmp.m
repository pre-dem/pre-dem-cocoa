...
- (NSString *)description {
  if (_code == 0) {
    return [NSString stringWithFormat:@"tcp connect min/avg/max = %f/%f/%fms",
                                      _minTime, _avgTime, _maxTime];
  }
...
- (void)run {
  ...
  NSTimeInterval *intervals =
      (NSTimeInterval *)malloc(sizeof(NSTimeInterval) * _count);
  NSInteger index = 0;
  NSInteger r = 0;
  do {
    ...
    if (r == 0) {
      [self.output
          write:[NSString stringWithFormat:@"rtmp handshake to %s:%lu, %f ms\n",
                                           inet_ntoa(addr.sin_addr),
                                           (unsigned long)_port,
                                           duration * 1000]];
    } else {
      [self.output
          write:[NSString
                    stringWithFormat:
                        @"rtmp handshake failed to %s:%lu, %f ms, error %ld\n",
                        inet_ntoa(addr.sin_addr), (unsigned long)_port,
                        duration * 1000, (long)r]];
    }
...
- (QNNRtmpHandshakeResult *)buildResult:(NSInteger)code
  ...
  return [[QNNRtmpHandshakeResult alloc] init:code
                                          max:max
                                          min:min
                                          avg:avg
                                        count:count];
...
- (NSInteger)handShake:(struct sockaddr_in *)addr start:(NSDate *)start {
  ...
  [self.output
      write:[NSString stringWithFormat:@"rtmp connect to %s:%lu, %f ms\n",
                                       inet_ntoa(addr->sin_addr),
                                       (unsigned long)_port, duration * 1000]];
...
+ (instancetype)start:(NSString *)host
               output:(id<QNNOutputDelegate>)output
             complete:(QNNRtmpHandshakeCompleteHandler)complete {
  return [QNNRtmpHandshake start:host
                            port:1935
                           count:2
                          output:output
                        complete:complete];
