...
NSString *const UICKeyChainStoreErrorDomain =
...
+ (UICKeyChainStore *)keyChainStoreWithServer:(NSURL *)server
                                 protocolType:(UICKeyChainStoreProtocolType)
                                                  protocolType {
  return
      [[self alloc] initWithServer:server
                      protocolType:protocolType
                authenticationType:UICKeyChainStoreAuthenticationTypeDefault];
}

+ (UICKeyChainStore *)
keyChainStoreWithServer:(NSURL *)server
...
- (instancetype)initWithServer:(NSURL *)server
                  protocolType:(UICKeyChainStoreProtocolType)protocolType
            authenticationType:
                (UICKeyChainStoreAuthenticationType)authenticationType {
  self = [super init];
  if (self) {
...
+ (NSString *)stringForKey:(NSString *)key
               ...
               accessGroup:(NSString *)accessGroup {
  return
      [self stringForKey:key service:service accessGroup:accessGroup error:nil];
}
...
+ (NSString *)stringForKey:(NSString *)key
  ...
  if (!key) {
    NSError *e = [self
        argumentError:NSLocalizedString(@"the key must not to be nil", nil)];
    if (error) {
      *error = e;
    }
    return nil;
  ...
  UICKeyChainStore *keychain =
      [UICKeyChainStore keyChainStoreWithService:service
...
+ (BOOL)setString:(NSString *)value
 ...
 genericAttribute:(id)genericAttribute
  ...
  NSError *e = [self
      conversionError:NSLocalizedString(@"failed to convert string to data",
                                        nil)];
  if (error) {
    *error = e;
  }
...
+ (NSData *)dataForKey:(NSString *)key
           ...
           accessGroup:(NSString *)accessGroup {
  return
      [self dataForKey:key service:service accessGroup:accessGroup error:nil];
}
...
- (NSString *)stringForKey:(id)key error:(NSError *__autoreleasing *)error {
  ...
  if (data) {
    ...
    NSError *e =
        [self.class conversionError:NSLocalizedString(
...
- (NSData *)dataForKey:(NSString *)key error:(NSError *__autoreleasing *)error {
  ...
  if (status == errSecSuccess) {
    ...
    if (data) {
      ...
      NSError *e =
          [self.class unexpectedError:NSLocalizedString(
...
- (BOOL)setData:(NSData *)data
  ...
  if (!key) {
    NSError *e =
        [self.class argumentError:NSLocalizedString(
                                      @"the key must not to be nil", nil)];
    if (error) {
      *error = e;
    }
    return NO;
  ...
  if (status == errSecSuccess || status == errSecInteractionNotAllowed) {
    ...
    NSMutableDictionary *attributes =
        [self attributesWithKey:nil value:data error:&unexpectedError];
    ...
    if (unexpectedError) {
      NSLog(@"error: [%@] %@", @(unexpectedError.code),
            NSLocalizedString(@"Unexpected error has occurred.", nil));
      if (error) {
      ...
      if (status == errSecInteractionNotAllowed &&
          floor(NSFoundationVersionNumber) <= floor(1140.11)) { // iOS 8.0.x
    ...
    NSError *unexpectedError = nil;
    NSMutableDictionary *attributes =
        [self attributesWithKey:key value:data error:&unexpectedError];
...
+ (BOOL)removeItemForKey:(NSString *)key
                 service:(NSString *)service
                   error:(NSError *__autoreleasing *)error {
  return
      [self removeItemForKey:key service:service accessGroup:nil error:error];
}
...
+ (BOOL)removeAllItemsForService:(NSString *)service
                     accessGroup:(NSString *)accessGroup {
  return
      [self removeAllItemsForService:service accessGroup:accessGroup error:nil];
}
...
- (NSArray UIC_KEY_TYPE *)allKeys {
  NSArray *items =
      [self.class prettify:[self itemClassObject] items:[self items]];
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:
    (UICKeyChainStoreItemClass)itemClass {
  ...
  query[(__bridge __strong id)kSecReturnAttributes] =
      (__bridge id)kCFBooleanTrue;
  ...
  if (status == errSecSuccess) {
    NSArray *items =
        [self prettify:itemClassObject items:(__bridge NSArray *)result];
    NSMutableArray *keys = [[NSMutableArray alloc] init];
    for (NSDictionary *item in items) {
  ...
  for (NSDictionary *attributes in items) {
    ...
    if (itemClass == kSecClassGenericPassword) {
      ...
      id authenticationType =
          attributes[(__bridge id)kSecAttrAuthenticationType];
...
- (void)setSynchronizable:(BOOL)synchronizable {
  ...
  if (_authenticationPolicy) {
    NSLog(@"%@",
          @"Cannot specify both an authenticationPolicy and a synchronizable");
  }
}

- (void)setAccessibility:(UICKeyChainStoreAccessibility)accessibility
    authenticationPolicy:
        (UICKeyChainStoreAuthenticationPolicy)authenticationPolicy {
...
#if TARGET_OS_IOS
- (void)sharedPasswordWithCompletion:(void (^)(NSString *account,
                                               NSString *password,
  ...
  if (domain.length > 0) {
    [self.class requestSharedWebCredentialForDomain:domain
                                            account:nil
                                         completion:^(NSArray *credentials,
                                                      NSError *error) {
    ...
    NSError *error =
        [self.class argumentError:NSLocalizedString(
                                      @"the server property must not to be "
                                      @"nil, should use "
                                      @"'keyChainStoreWithServer:protocolType:"
                                      @"' initializer to instantiate keychain "
                                      @"store",
                                      nil)];
    if (completion) {
      completion(nil, nil, error);
...
- (void)sharedPasswordForAccount:(NSString *)account
                      completion:(void (^)(NSString *password,
                                           NSError *error))completion {
  NSString *domain = self.server.host;
  if (domain.length > 0) {
    [self.class requestSharedWebCredentialForDomain:domain
...
- (void)setSharedPassword:(NSString *)password
  ...
  if (domain.length > 0) {
    SecAddSharedWebCredential(
        (__bridge CFStringRef)domain, (__bridge CFStringRef)account,
...
- (void)removeSharedPasswordForAccount:(NSString *)account
                            completion:(void (^)(NSError *error))completion {
  [self setSharedPassword:nil forAccount:account completion:completion];
}

+ (void)requestSharedWebCredentialWithCompletion:
    (void (^)(NSArray UIC_CREDENTIAL_TYPE *credentials,
              NSError *error))completion {
  [self requestSharedWebCredentialForDomain:nil
                                    account:nil
                                 completion:completion];
}

+ (void)
requestSharedWebCredentialForDomain:(NSString *)domain
  ...
  SecRequestSharedWebCredential(
      ...
      ^(CFArrayRef credentials, CFErrorRef error) {
        ...
        for (NSDictionary *credential in (__bridge NSArray *)credentials) {
          ...
          NSString *password =
              credential[(__bridge __strong id)kSecSharedPassword];
          if (password) {
...
- (NSMutableDictionary *)query {
  ...
  if (floor(NSFoundationVersionNumber) >
      floor(993.00)) { // iOS 7+ (NSFoundationVersionNumber_iOS_6_1)
    query[(__bridge __strong id)kSecAttrSynchronizable] =
  ...
  if (itemClass == kSecClassGenericPassword) {
    ...
    if (protocolTypeObject) {
      query[(__bridge __strong id)kSecAttrProtocol] =
          (__bridge id)protocolTypeObject;
    }
    CFTypeRef authenticationTypeObject = [self authenticationTypeObject];
    if (authenticationTypeObject) {
      query[(__bridge __strong id)kSecAttrAuthenticationType] =
          (__bridge id)authenticationTypeObject;
    }
  }
  ...
  if (_authenticationPrompt) {
    if (floor(NSFoundationVersionNumber) >
        floor(1047.25)) { // iOS 8+ (NSFoundationVersionNumber_iOS_7_1)
      query[(__bridge __strong id)kSecUseOperationPrompt] =
          _authenticationPrompt;
    } else {
      NSLog(@"%@", @"Unavailable 'authenticationPrompt' attribute on iOS "
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key
  ...
  if (_authenticationPolicy && accessibilityObject) {
    if (floor(NSFoundationVersionNumber) >
        floor(iOS_7_1_or_10_9_2)) { // iOS 8+ or OS X 10.10+
      CFErrorRef securityError = NULL;
      SecAccessControlRef accessControl = SecAccessControlCreateWithFlags(
          kCFAllocatorDefault, accessibilityObject,
      ...
      attributes[(__bridge __strong id)kSecAttrAccessControl] =
          (__bridge_transfer id)accessControl;
    } else {
#if TARGET_OS_IOS
      NSLog(
          @"%@",
          @"Unavailable 'Touch ID integration' on iOS versions prior to 8.0.");
#else
      NSLog(@"%@", @"Unavailable 'Touch ID integration' on OS X versions prior "
                   @"to 10.10.");
#endif
    }
  } else {
...
#if TARGET_OS_IOS
      NSLog(@"%@", @"Unavailable "
                   @"'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOn"
                   @"ly' attribute on iOS versions prior to 8.0.");
#else
      NSLog(@"%@", @"Unavailable "
                   @"'UICKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOn"
                   @"ly' attribute on OS X versions prior to 10.10.");
#endif
    } else {
      if (accessibilityObject) {
        attributes[(__bridge __strong id)kSecAttrAccessible] =
            (__bridge id)accessibilityObject;
      }
    }
  }

  if (floor(NSFoundationVersionNumber) > floor(993.00)) { // iOS 7+
    attributes[(__bridge __strong id)kSecAttrSynchronizable] =
        @(_synchronizable);
  }
...
+ (NSError *)argumentError:(NSString *)message {
  NSError *error =
      [NSError errorWithDomain:UICKeyChainStoreErrorDomain
