...
- (void)setPath:(NSString *)path {
  ...
  [pathsComponents enumerateObjectsUsingBlock:^(id obj, NSUInteger idx,
                                                BOOL *stop) {
    // 第一部分为空，舍去
    if (idx > 0 && idx < 5) {
      [self
          setValue:obj
            forKey:[NSString stringWithFormat:@"path%lu", (unsigned long)idx]];
    }
  }];
  ...
  if (pathsComponents.count > 5) {
    ...
    _path4 = [path
        substringFromIndex:(_path1.length + _path2.length + _path3.length + 4)];
  }
}
