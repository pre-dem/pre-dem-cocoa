...
@interface PREDURLProtocol () <NSURLSessionDataDelegate>
@end
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
  return ![NSURLProtocol propertyForKey:kInternalRequestPropertyKey
                              inRequest:request] &&
...
+ (NSURLRequest *)canonicalRequestForRequest:(NSURLRequest *)request {
  ...
  if (request.URL.port) {
    HTTPMonitorModel.domain = [NSString
        stringWithFormat:@"%@:%@", HTTPMonitorModel.domain, request.URL.port];
  } else {
  ...
  NSError *error;
  NSData *queryData = [NSJSONSerialization
      dataWithJSONObject:[PREDHelper parseQuery:request.URL.query]
                 options:0
                   error:&error];
  if (!error) {
    HTTPMonitorModel.query =
        [[NSString alloc] initWithData:queryData encoding:NSUTF8StringEncoding];
  }
  ...
  if ([request.URL.scheme isEqualToString:@"http"]) {
    ...
    NSString *hostIP =
        [PREDHelper lookupHostIPAddressForURL:mutableRequest.URL];
    ...
    NSInteger number =
        [regex numberOfMatchesInString:hostIP
                               options:0
                                 range:NSMakeRange(0, [hostIP length])];
    if (number == 0) {
      return mutableRequest;
    }

    NSString *otherComponents = [mutableRequest.URL.absoluteString
        substringFromIndex:mutableRequest.URL.scheme.length + 3 +
                           mutableRequest.URL.host.length];
    NSURL *replacedURL = [NSURL
        URLWithString:[NSString stringWithFormat:@"%@://%@%@",
                                                 mutableRequest.URL.scheme,
                                                 hostIP, otherComponents]];
    HTTPMonitorModel.dns_time =
...
- (void)startLoading {
  PREDHTTPMonitorModel *HTTPMonitorModel =
      [NSURLProtocol propertyForKey:kHTTPMonitorModelPropertyKey
                          inRequest:self.request];
  HTTPMonitorModel.start_timestamp =
      (UInt64)([[NSDate date] timeIntervalSince1970] * 1000);
  HTTPMonitorModel.end_timestamp =
      (UInt64)([[NSDate date] timeIntervalSince1970] * 1000);
  HTTPMonitorModel.response_time_stamp =
      (UInt64)([[NSDate date] timeIntervalSince1970] * 1000);
  ...
  [task resume];
  [NSURLProtocol setProperty:task
                      forKey:kTaskPropertyKey
                   inRequest:(NSMutableURLRequest *)self.request];
  [session finishTasksAndInvalidate];
}

- (void)stopLoading {
  NSURLSessionTask *task =
      [NSURLProtocol propertyForKey:kTaskPropertyKey inRequest:self.request];
  [task cancel];
}
