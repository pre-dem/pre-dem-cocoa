...
- (void)postPath:(NSString *)path
  ...
  NSMutableURLRequest *request = [NSMutableURLRequest
      requestWithURL:[NSURL URLWithString:path relativeToURL:_baseURL]];
  ...
  if (headers) {
    [headers enumerateKeysAndObjectsUsingBlock:^(
      ...
      NSAssert([key isKindOfClass:[NSString class]],
               @"headers can only be string-string pairs");
...
- (void)sendRequest:(NSMutableURLRequest *)request
  ...
  PREDHTTPOperation *operation = [self
      operationWithURLRequest:request
                   completion:^(PREDHTTPOperation *operation, NSData *data,
                                NSError *error) {
                     if (operation.response.statusCode >= 400) {
                       error = [PREDError
                           GenerateNSError:kPREDErrorCodeInternalError
                               description:
                                   @"server returned an error status code: %d, "
                                   @"body: %@",
...
- (PREDHTTPOperation *)operationWithURLRequest:(NSURLRequest *)request
                                    completion:
                                        (PREDNetworkCompletionBlock)completion {
  PREDHTTPOperation *operation =
      [PREDHTTPOperation operationWithRequest:request];
...
- (void)authorizeRequest:(NSMutableURLRequest *)request {
  ...
  NSString *auth =
      [PREDCredential authorize:domainPath
                         appKey:[[PREDManager sharedPREDManager] appKey]];
