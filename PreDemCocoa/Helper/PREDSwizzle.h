...
//  Original implementation by Yan Rabovik on 05.09.13
...
/// May be used only in PREDSwizzleInstanceMethod or PREDSwizzleClassMethod
/// macros.
...
/**
 ...
 one of its superclasses) has been already swizzled. Here the `PREDSwizzleMode`
 and `key` parameters can help. See +[PREDSwizzle
 ...
 @param PREDSWReturnType The return type of the swizzled method wrapped in the
 PREDSWReturnType macro.

 @param PREDSWArguments The arguments of the swizzled method wrapped in the
 PREDSWArguments macro.

 @param PREDSWReplacement The code of the new implementation of the swizzled
 method wrapped in the PREDSWReplacement macro.

 @param PREDSwizzleMode The mode is used in combination with the key to indicate
 ...
 */
#define PREDSwizzleInstanceMethod(classToSwizzle, selector, PREDSWReturnType,  \
                                  PREDSWArguments, PREDSWReplacement,          \
                                  PREDSwizzleMode, key)                        \
  _PREDSwizzleInstanceMethod(classToSwizzle, selector, PREDSWReturnType,       \
 ...
 */
#define PREDSwizzleClassMethod(classToSwizzle, selector, PREDSWReturnType,     \
                               PREDSWArguments, PREDSWReplacement)             \
  _PREDSwizzleClassMethod(classToSwizzle, selector, PREDSWReturnType,          \
                          _PREDSWWrapArg(PREDSWArguments),                     \
                          _PREDSWWrapArg(PREDSWReplacement))
...
/**
 PREDSwizzleInfo is used in the new implementation block to get and call
...
typedef NS_ENUM(NSUInteger, PREDSwizzleMode) {
  ...
  /// PREDSwizzle does not do swizzling if the same class has been swizzled
  /// earlier with the same key.
  PREDSwizzleModeOncePerClass = 1,
  /// PREDSwizzle does not do swizzling if the same class or one of its
...
#define _PREDSwizzleInstanceMethod(classToSwizzle, selector, PREDSWReturnType, \
                                   PREDSWArguments, PREDSWReplacement,         \
                                   PREDSwizzleMode, KEY)                       \
  [PREDSwizzle                                                                 \
      swizzleInstanceMethod:selector                                           \
              ...
              newImpFactory:^id(PREDSwizzleInfo *swizzleInfo) {                \
                PREDSWReturnType (*originalImplementation_)(_PREDSWDel3Arg(    \
                    __unsafe_unretained id, SEL, PREDSWArguments));            \
                SEL selector_ = selector;                                      \
                return ^PREDSWReturnType(_PREDSWDel2Arg(                       \
                    __unsafe_unretained id self, PREDSWArguments)) {           \
                  PREDSWReplacement                                            \
                };                                                             \
              }                                                                \
...
#define _PREDSwizzleClassMethod(classToSwizzle, selector, PREDSWReturnType,    \
                                PREDSWArguments, PREDSWReplacement)            \
  [PREDSwizzle                                                                 \
      swizzleClassMethod:selector                                              \
                 inClass:[classToSwizzle class]                                \
...
#define _PREDSWCallOriginal(arguments...)                                      \
  ((__typeof(                                                                  \
      originalImplementation_))[swizzleInfo getOriginalImplementation])(       \
