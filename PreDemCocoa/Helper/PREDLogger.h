...
#define PREDLog(_level, _message)                                              \
  [PREDLogger logMessage:_message                                              \
                   level:_level                                                \
                    file:__FILE__                                              \
                function:__PRETTY_FUNCTION__                                   \
                    line:__LINE__]

#define PREDLogError(format, ...)                                              \
  PREDLog(PREDLogLevelError, (^{                                               \
            return [NSString stringWithFormat:(format), ##__VA_ARGS__];        \
          }))
#define PREDLogWarning(format, ...)                                            \
  PREDLog(PREDLogLevelWarning, (^{                                             \
            return [NSString stringWithFormat:(format), ##__VA_ARGS__];        \
          }))
#define PREDLogDebug(format, ...)                                              \
  PREDLog(PREDLogLevelDebug, (^{                                               \
            return [NSString stringWithFormat:(format), ##__VA_ARGS__];        \
          }))
#define PREDLogVerbose(format, ...)                                            \
  PREDLog(PREDLogLevelVerbose, (^{                                             \
            return [NSString stringWithFormat:(format), ##__VA_ARGS__];        \
          }))
