...
+ (NSString *)readUUIDFromKeyChain {
  UICKeyChainStore *keychain =
      [UICKeyChainStore keyChainStoreWithService:kPREDKeychainServiceName];
...
+ (NSString *)generateNewUUIDString {
  ...
  NSString *uuidString =
      [NSString stringWithString:(__bridge NSString *)strRef];
  CFRelease(strRef);
  CFRelease(uuidRef);
  UICKeyChainStore *keychain =
      [UICKeyChainStore keyChainStoreWithService:kPREDKeychainServiceName];
...
+ (NSString *)sdkDirectory {
  return [NSString stringWithFormat:@"%@%@",
                                    [[[[NSFileManager defaultManager]
                                        ...
                                        absoluteString] substringFromIndex:7],
                                    kPREDDirectoryName];
}
...
+ (NSMutableDictionary *)parseQuery:(NSString *)query {
  ...
  for (NSString *keyValuePair in urlComponents) {
    ...
    NSString *key =
        [[pairComponents firstObject] stringByRemovingPercentEncoding];
    NSString *value =
        [[pairComponents lastObject] stringByRemovingPercentEncoding];
...
http://deusty.blogspot.com/2007/07/gzip-compressiondecompression.html
  ...
  if (!pUncompressedData || [pUncompressedData length] == 0) {
    if (error) {
      *error = [PREDError
          GenerateNSError:kPREDErrorCodeCompressionError
              description:
  ...
  /* Initialize the zlib deflation (i.e. compression) internals with
   ...
   int level      - Compression level. Must be Z_DEFAULT_COMPRESSION, or between
                    0 and 9: 1 gives best speed, 9 gives best compression, 0
   gives
                    no compression.
   ...
   int windowBits - Base two logarithm of the maximum window size (the size of
                    ...
                    gzip header will have no file name, no extra data, no
   comment,
   ...
   int memLevel   - Amount of memory allocated for internal compression state.
                    ...
                    ratio; 9 uses maximum memory for optimal speed. Default
   value
                    is 8.
  ...
  if (initError != Z_OK) {
    ...
    case Z_VERSION_ERROR:
      errorMsg = @"The version of zlib.h and the version of the library linked "
                 @"do not match.";
      break;
    default:
    ...
    if (error) {
      *error = [PREDError
          GenerateNSError:kPREDErrorCodeCompressionError
              description:@"%s: deflateInit2() Error: \"%@\" Message: \"%s\"",
                          __func__, errorMsg, zlibStreamStruct.msg];
    }
    return nil;
  }

  // Create output memory buffer for compressed data. The zlib documentation
  // states that
  // destination buffer size must be at least 0.1% larger than avail_in plus 12
  // bytes.
  NSMutableData *compressedData = [NSMutableData
      dataWithLength:(NSUInteger)([pUncompressedData length] * 1.01 + 12)];
  ...
  if (deflateStatus != Z_STREAM_END) {
    ...
    if (error) {
      *error =
          [PREDError GenerateNSError:kPREDErrorCodeCompressionError
                         description:@"%s: zlib error while attempting "
                                     @"compression: \"%@\" Message: \"%s\"",
                                     __func__, errorMsg, zlibStreamStruct.msg];
    }
