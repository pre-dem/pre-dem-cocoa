 ...
 */
FOUNDATION_EXPORT NSString *const PREDChannelBlockedNotification;

/**
 *  Telemetry context used by the channel to create the payload (testing).
 */
@property (nonatomic, strong) PREDTelemetryContext *telemetryContext;
...
/**
 *  Manually trigger the PREDChannel to persist all items currently in its data item queue.
 */
